class User < ActiveRecord::Base
  ...
  def project_count
    Project.where('author_id = ?', id).count + Position.select(:project_id).distinct.includes(:project).references(:project).where('user_id = ? and projects.author_id != ?', id, id).count
  end
